...
def test_mini_case():
    result = subprocess.run(
        [sys.executable, "parse_mutations.py", "--file", "test_files/mini.tsv"],
        capture_output=True,
        text=True
    )
...
def test_run_no_data():
    result = subprocess.run(
        [sys.executable, "parse_mutations.py", "--file", "test_files/no_data.tsv"],
        capture_output=True,
        text=True
    )
...
def test_missing_mutation_id():
    result = subprocess.run(
        [sys.executable, "parse_mutations.py", "--file", "test_files/missing_mutation.tsv"],
        capture_output=True,
        text=True
    )
